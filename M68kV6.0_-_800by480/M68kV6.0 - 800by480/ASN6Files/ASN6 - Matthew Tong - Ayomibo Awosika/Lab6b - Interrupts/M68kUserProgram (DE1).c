...
/***Global Variables for Timer ISR*/
unsigned char Timer8ISRCount;
...
void main()
{
    ...
    IQR_Check = 0;
    while(1){
        if(Timer8ISRCount != IQR_Check){
            printf("|==========MEASURING==========|\n");
            if (Timer8ISRCount % 1 == 0){
                switches = (PortB << 8) | (PortA);

                CanBus0_Transmit(switches);
                CanBus1_Receive(SWITCHES);
            }

            if (Timer8ISRCount % 2 == 0){
                potential = ADCWrite(POTENTIAL);

                CanBus0_Transmit(potential) ;
                CanBus1_Receive(POTENTIAL) ;
            }

            if (Timer8ISRCount % 5 == 0){
                light = ADCWrite(LIGHT);

                CanBus0_Transmit(light) ;
                CanBus1_Receive(LIGHT) ;
            }

            if (Timer8ISRCount % 20 == 0){
                temp = ADCWrite(TEMP);

                CanBus0_Transmit(temp) ;
                CanBus1_Receive(TEMP) ;
            }

            IQR_Check = Timer8ISRCount;
